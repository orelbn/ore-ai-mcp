...
function requiredHeader(request: Request, headerName: string): string {
  ...
  if (!value) {
    throw new AppError("UNAUTHENTICATED", `Missing required header: ${headerName}`, 401);
  }
  return value;
}

export function authenticateRequest(request: Request, env: Env): AuthenticatedCaller {
  ...
  if (!requestId) {
    throw new AppError("INVALID_INPUT", `Missing required header: ${HEADER_REQUEST_ID}`, 400);
  }
