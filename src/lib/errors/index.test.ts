...
import { AppError, normalizeError, toHttpErrorResponse, toToolErrorResult } from "./index";

describe("error handling", () => {
  it("sanitizes INTERNAL_ERROR in tool responses", () => {
    const result = toToolErrorResult(new AppError("INTERNAL_ERROR", "sensitive stack detail", 500));
  ...
  it("keeps non-internal messages in tool responses", () => {
    const result = toToolErrorResult(new AppError("INVALID_INPUT", "Missing required field", 400));
    expect(result.structuredContent).toMatchObject({
      ok: false,
      error: {
        code: "INVALID_INPUT",
