...
function registerTool(server: McpServer, context: RequestContext, options: ToolRegistration): void {
  server.registerTool(
    options.name,
...
function createToolRegistrations(config: GitHubInsightsConfig): ToolRegistration[] {
  return [
    {
...
export function registerGitHubTools(server: McpServer, context: RequestContext): void {
  ...
  for (const registration of createToolRegistrations(config)) {
    registerTool(server, context, registration);
  }
}
