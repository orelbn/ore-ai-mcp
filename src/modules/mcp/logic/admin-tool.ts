...
import { getContextToolInventory, saveDisabledToolOverrides } from "@/modules/context";
import {
  MCP_ROUTE,
...
function requireToolNames(
  ...
  if (normalizedToolNames.length === 0) {
    throw new AppError("INVALID_INPUT", `toolNames is required for action "${action}"`, 400);
  }
...
function validateKnownContextTools(
  ...
  const unknownToolNames = requestedToolNames.filter((toolName) => !knownToolNames.has(toolName));
  if (unknownToolNames.length > 0) {
    throw new AppError("INVALID_INPUT", `Unknown tool names: ${unknownToolNames.join(", ")}`, 400);
  }
}
...
export async function handleAdminTool(
  ...
  if (args.action === "status") {
    ...
    return toSuccessResult(`Loaded MCP server status for ${ORE_MCP_SERVER_NAME}.`, {
      action: "status",
  ...
  if (args.action === "list-tools") {
    const payload = await getInventoryPayload(context);
    return toSuccessResult(`Listed ${payload.tools.length} MCP tools for ${ORE_MCP_SERVER_NAME}.`, {
      action: "list-tools",
      server: payload.server,
      context: payload.context,
      disabledTools: payload.disabledTools,
  ...
  const savedConfig = await saveDisabledToolOverrides(
    context,
    inventory.disabledTools.config.filter((toolName) => !requestedToolNames.includes(toolName)),
  );
  const nextInventory = await getContextToolInventory(context);

  return toSuccessResult(`Enabled ${requestedToolNames.length} MCP tool(s) in runtime overrides.`, {
    action: "enable-tools",
    requestedToolNames,
    config: savedConfig,
    disabledTools: nextInventory.disabledTools,
