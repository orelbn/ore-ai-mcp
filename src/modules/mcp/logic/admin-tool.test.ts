    ...
    [CONTEXT_INDEX_KEY]: JSON.stringify({
      ...
      tools: {
        "ore.context.design_doc_to_issues": {
          ...
          description: "Break approved design docs into execution-ready issues.",
...
describe("admin tool", () => {
  it("returns server status and merged disabled-tool state", async () => {
    ...
    expect(result.structuredContent).toMatchObject({
      ...
      disabledTools: {
        ...
        combined: ["ore.context.design_doc_to_issues", "ore.context.issue_triage"],
      },
    });
  });
  ...
  it("persists runtime disable overrides and can clear them", async () => {
    ...
    expect(disableResult.structuredContent).toMatchObject({
      ...
      disabledTools: {
        config: ["ore.context.design_doc_to_issues", "ore.context.issue_triage"],
      },
    });
    ...
    expect(clearResult.structuredContent).toMatchObject({
      ok: true,
      action: "clear-overrides",
      clearedToolNames: ["ore.context.design_doc_to_issues", "ore.context.issue_triage"],
      disabledTools: {
        config: [],
      },
    });
  ...
  it("registers the admin tool and excludes disabled context tools from the live server", async () => {
    const server = await createOreMcpServer(makeContext(createBucketWithTools()));
