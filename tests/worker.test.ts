...
import { HEADER_INTERNAL_SECRET, HEADER_REQUEST_ID, HEADER_USER_ID } from "@/lib/auth";
...
describe("worker entrypoint", () => {
  ...
  it("handles stateless tools/list requests without initialize", async () => {
    ...
    const payloadLine = bodyText.split("\n").find((line) => line.startsWith("data: "));
    ...
    expect(body.result?.tools?.some((tool) => tool.name === "ore.server.manage")).toBe(true);
  });
