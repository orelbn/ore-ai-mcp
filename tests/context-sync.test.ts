...
describe("context sync planning", () => {
  ...
  it("fails to load manifest when explicit toolName has invalid characters", () => {
    const repoRoot = makeTempRepo();
    writeFileSync(join(repoRoot, ".context", "notes", "coffee", "top.md"), "# Coffee");
    writeFileSync(
      join(repoRoot, ".context", "context-manifest.json"),
      JSON.stringify(
        {
    ...
    expect(() => loadManifest(repoRoot)).toThrow("toolName must use [a-z0-9._-]");
  ...
  it("fails validation when image file is missing", () => {
    ...
    const manifest = loadManifest(repoRoot);
    const issues = validateManifest(repoRoot, manifest);
    expect(issues.some((issue) => issue.path === "images/missing.jpg")).toBe(true);
  ...
  it("fails validation for duplicate ids and unsafe paths", () => {
    ...
    const manifest = loadManifest(repoRoot);
    const issues = validateManifest(repoRoot, manifest);
    expect(issues.some((issue) => issue.message.includes("Duplicate contextId"))).toBe(true);
    expect(issues.some((issue) => issue.message.includes("Duplicate toolName mapping"))).toBe(true);
    expect(issues.some((issue) => issue.message.includes("safe relative path"))).toBe(true);
  ...
  it("builds index and managed keys", () => {
    const repoRoot = makeTempRepo();
    writeFileSync(join(repoRoot, ".context", "notes", "coffee", "top.md"), "# Coffee");
    writeFileSync(join(repoRoot, ".context", "images", "latte.jpg"), "image-bytes");
    writeFileSync(
      join(repoRoot, ".context", "context-manifest.json"),
      JSON.stringify(
        {
    ...
    const manifest = loadManifest(repoRoot);
    const artifacts = buildArtifacts(repoRoot, manifest, "2026-02-24T00:00:00.000Z");

    expect(artifacts.index.tools["ore.context.orel_top_coffee_shops"]?.markdownKey).toBe(
      "context/markdown/orel-top-coffee-shops.md",
    );
    expect(artifacts.index.managedKeys.includes(CONTEXT_INDEX_KEY)).toBe(true);
  ...
  it("plans mirror deletes for removed objects", () => {
    const previous = ["context/markdown/old.md", "context/images/old.jpg", CONTEXT_INDEX_KEY];
    const next = ["context/markdown/new.md", CONTEXT_INDEX_KEY];

    const deletions = planMirrorDeletes(previous, next);
    expect(deletions).toEqual(["context/images/old.jpg", "context/markdown/old.md"]);
  });
});
