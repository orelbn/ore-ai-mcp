...
/**
 * Read and validate the remote project override index from the Cloudflare KV binding for the specified environment.
 *
 * @param repoRoot - Repository root used to execute Wrangler commands
 * @param syncArgs - Parsed CLI sync arguments (controls environment and Wrangler invocation)
 * @returns The parsed ProjectInsightOverrideIndex, or `null` if the index key is not present in KV
 * @throws If a remote index is present but its schema is invalid, or if Wrangler returns an unexpected error
 */
...
/**
 * Synchronizes local project override files with the remote Cloudflare KV store based on CLI arguments.
 *
 * Builds the next override index from local overrides, reads and validates the remote index, computes keys to delete,
 * and either prints a dry-run plan or performs the apply sequence which uploads new/updated overrides and deletes removed keys.
 * A temporary index file is created during apply and is removed on completion.
 */
async function main() {
