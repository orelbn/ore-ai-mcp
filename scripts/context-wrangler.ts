...
function stripComments(jsonc: string): string {
  return jsonc.replaceAll(/\/\*[\s\S]*?\*\//g, "").replaceAll(/(^|\s)\/\/.*$/gm, "$1");
}
...
export function resolveBucketName(
  ...
  const config = parseWranglerConfig(raw) as {
    ...
    env?: Record<string, { r2_buckets?: Array<{ binding: string; bucket_name: string }> }>;
  };

  const buckets =
    envName && envName.length > 0 ? config.env?.[envName]?.r2_buckets : config.r2_buckets;

  if (!buckets || buckets.length === 0) {
    throw new Error(`No r2_buckets configured for ${envName ? `env ${envName}` : "top-level"}`);
  }
...
export function runWrangler(args: string[], cwd: string): string {
  ...
  throw new Error(`Wrangler command failed: bunx wrangler ${args.join(" ")}\n${stderr || stdout}`);
}
