...
export async function fetchConfig(): Promise<LocalDevClientConfig> {
	...
	const payload = await parseJson<{ ok: true; config: LocalDevClientConfig }>(
		response,
	);
...
export async function fetchToolContent(
	...
	const response = await fetch(
		"/api/tool-content",
		createRequestBody({ toolName }),
	);
	const payload = await parseJson<{ ok: true; result: ToolContentResponse }>(
		response,
	);
