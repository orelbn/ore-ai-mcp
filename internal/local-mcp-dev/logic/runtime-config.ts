...
async function detectLocalMcpUrl(
	...
	for (const candidate of candidates) {
		if (
			await probeLocalMcpUrl(candidate, secret, callerWorker, userId, fetchImpl)
		) {
			return candidate;
		}
	}
...
export async function buildDashboardRuntimeConfig(
	...
	const defaultCallerWorker =
		env.INTERNAL_MCP_CALLER_WORKER?.trim() || "ore-ai";
	const defaultUserId = env.INTERNAL_MCP_USER_ID?.trim() || "local-mcp-dev";
	const sharedSecret = resolveSharedSecret(env, devVars);

	if (!sharedSecret) {
		return {
			port: parsePort(env.INTERNAL_MCP_ADMIN_PORT),
			localUrl: null,
			setupMessage:
				"No local shared secret found. Add `MCP_INTERNAL_SHARED_SECRET` to `.dev.vars` and start `bun run dev`.",
